...
launchContract contract wllt = do
    ...
    void $ fork @effs @EmulatorMessage "contract instance" Normal (runReader wllt $ interpret (mapLog InstanceEvent) $ reinterpret (mapLog InstanceEvent) $ contractThread handle)
...
handleCallEndpoint wllt endpointName endpointValue = do
    ...
    void $ fork @effs @EmulatorMessage "call endpoint" Normal thr